...
export default function CreateArticle() {
  ...
  var numberOfSources = 1;
  const createArticleFunc = (e) => {
    //function handling submitting article
    e.preventDefault();
    ...
    for (let i = 0; i < numberOfSources; i++) {
      sources.push(document.getElementById(`create_article_title_${i}`).value);
    }
    const articleData = {
      img: document.getElementById("create_article_picture").value,
      title: document.getElementById("create_article_title").value,
      body: document.getElementById("create_article_body").value,
      hashtags: [document.getElementById("create_article_tag").value],
      sources,
    };
    console.log(articleData);
    requestTemplate(
      `${process.env.BACKEND_URI}/api/blogs`,
      "POST",
      "include",
      {},
      { ...articleData }
  ...
  const addNewSourceFunc = (e) => {
    ...
    document
      ...
      .insertAdjacentHTML("afterend", newInput);
  };

  return (
    <div className="row h-100">
      <form
        onSubmit={createArticleFunc}
        className={styles.createArticleForm + " col-lg-6"}
      >
        <h2 className="display-2">Write article</h2>
        <input
          id="create_article_picture"
        ...
        <label className="form-check-label px-2">
          I confirm that the informations provided are verified
        </label>
        <button className="btn btn-primary text-light p-2 my-2" type="submit">
          Create article
        </button>
      </form>
      {window.innerWidth > 991 && (
        <div className="col-lg-6">
          ...
          <ReactMarkdown
            className={styles.createArticlePreview}
